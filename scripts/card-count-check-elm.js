	...
	function (red, player, cardId, hand) {
		...
		var remove = function (list) {
			if (!list.b) {
				...
				if (x.$ === 'Just') {
					var c = x.a;
					return _Utils_eq(c.id, cardId) ? rest : A2(
						$elm$core$List$cons,
						x,
						remove(rest));
				} else {
		...
		return _Utils_eq(player, red) ? _Utils_update(
			hand,
			{
				red: attempt(hand.red)
			}) : _Utils_update(
			hand,
			{
				blue: attempt(hand.blue)
			});
	});
var $author$project$Main$removeN = F4(
	function (red, player, n, hand) {
		...
		return _Utils_eq(player, red) ? _Utils_update(
			hand,
			{
				red: A2(dropLast, n, hand.red)
			}) : _Utils_update(
			hand,
			{
				blue: A2(dropLast, n, hand.blue)
			});
	});
	...
	function (red, hand, detail) {
		...
		switch (_v0.$) {
			...
			case 'DrewCard':
				var player = _v0.a.player;
				var card = _v0.a.card;
				var andPlayed = _v0.a.andPlayed;
				if (andPlayed.$ === 'Nothing') {
					return A4(
						$author$project$Main$addCard,
						red,
						player,
			...
			case 'Attached':
				var player = _v0.a.player;
				var item = _v0.a.item;
				return A4($author$project$Main$removeById, red, player, item.id, hand);
			case 'DiscardedCard':
				var player = _v0.a.player;
				var card = _v0.a.card;
...
var $author$project$Main$pilesDeckDelta = F4(
	function (red, player, delta, piles) {
		return _Utils_eq(player, red) ? _Utils_update(
			piles,
			{deckRed: piles.deckRed + delta}) : _Utils_update(
			piles,
			{deckBlue: piles.deckBlue + delta});
	});
var $author$project$Main$pilesDiscardDelta = F4(
	function (red, player, delta, piles) {
		return _Utils_eq(player, red) ? _Utils_update(
			piles,
			{discardRed: piles.discardRed + delta}) : _Utils_update(
			piles,
			{discardBlue: piles.discardBlue + delta});
	});
var $author$project$Main$pilesPrizeDelta = F4(
	function (red, player, delta, piles) {
		return _Utils_eq(player, red) ? _Utils_update(
			piles,
			{prizesRed: piles.prizesRed + delta}) : _Utils_update(
			piles,
			{prizesBlue: piles.prizesBlue + delta});
	});
	...
	function (red, isSetup, action, piles) {
		switch (action.$) {
			...
			case 'PlayedPokemon':
				var player = action.a.player;
				var position = action.a.position;
				return (isSetup && _Utils_eq(position, $author$project$Action$ActiveSpot)) ? A4(
			...
			F2(
				function (detail, st) {
					return A3(
						...
						F2(
							function (bullet, bs) {
								return A2(applyAction, bullet.action, bs);
							}),
						A2(applyAction, detail.action, st),
						detail.bullets);
				}),
			A2(applyAction, group.action, stadium),
			group.details);
	});
...
var $author$project$Replay$ResultSection = function (a) {
	return {$: 'ResultSection', a: a};
};
var $author$project$Replay$TurnSection = F2(
	function (a, b) {
		return {$: 'TurnSection', a: a, b: b};
	});
		...
		return _Utils_ap(
			...
			_List_fromArray(
				[
					...
					kind,
					$elm$core$List$reverse(state.currentLines))
				]));
	}
};
...
var $author$project$Replay$identifyPlayers = function (sections) {
	if (sections.b && (sections.a.$ === 'SetupSection')) {
		...
		var _v1 = _Utils_Tuple2(
			...
			drewPlayers);
		if (((_v1.a.$ === 'Just') && _v1.b.b) && _v1.b.b.b) {
			var red = _v1.a.a;
			var _v2 = _v1.b;
			var p1 = _v2.a;
			var _v3 = _v2.b;
	...
	if (A2($elm$core$String$startsWith, 'Turn # ', line)) {
		...
		if ((_v0.b && _v0.b.b) && (!_v0.b.b.b)) {
			...
			return A2(
				...
				function (n) {
					return {
						maybeNumber: $elm$core$Maybe$Just(n),
						player: A2($elm$core$String$dropRight, 7, playerPart)
					};
				},
				$elm$core$String$toInt(
		...
		if (A2($elm$core$String$endsWith, '\'s Turn', line) && (!A2($elm$core$String$contains, '.', line))) {
			return $elm$core$Maybe$Just(
				{
					maybeNumber: $elm$core$Maybe$Nothing,
					player: A2($elm$core$String$dropRight, 7, line)
				});
		} else {
			return $elm$core$Maybe$Nothing;
		}
...
var $author$project$Replay$startSection = F2(
	function (kind, state) {
		return {
			currentKind: $elm$core$Maybe$Just(kind),
			currentLines: _List_Nil,
			sections: $author$project$Replay$commitSection(state),
	...
	function (line, state) {
		if (line === 'Setup') {
			...
			if (line === 'Pokémon Checkup') {
				...
				if (_v0.$ === 'Just') {
					var player = _v0.a.player;
					var maybeNumber = _v0.a.maybeNumber;
					var newCount = state.turnCount + 1;
					var turn = {
						number: A2($elm$core$Maybe$withDefault, newCount, maybeNumber),
						player: player
					};
					return {
						currentKind: $elm$core$Maybe$Just(
...
var $author$project$Replay$parse = function (raw) {
	...
	return {
		players: $author$project$Replay$identifyPlayers(sections),
		sections: sections
	};
};
var $elm$url$Url$percentEncode = _Url_percentEncode;
var $author$project$CardCountCheck$checkFile = function (flags) {
	var replay = $author$project$Replay$parse(flags.content);
	var _v0 = replay.players;
	if (_v0.$ === 'Nothing') {
		return {ok: false, output: 'ERROR  ' + (flags.name + '\n  could not identify players\n')};
	} else {
		var players = _v0.a;
...
var $author$project$CardCountCheck$done = _Platform_outgoingPort(
	'done',
	function ($) {
...
var $author$project$CardCountCheck$main = $elm$core$Platform$worker(
	{
		init: function (flags) {
			return _Utils_Tuple2(
				_Utils_Tuple0,
				$author$project$CardCountCheck$done(
					$author$project$CardCountCheck$checkFile(flags)));
		},
...
_Platform_export({'CardCountCheck':{'init':$author$project$CardCountCheck$main(
	A2(
					...
					return A2(
						...
						function (content) {
							return $elm$json$Json$Decode$succeed(
								{content: content, name: name, replayUrl: replayUrl});
						},
						A2($elm$json$Json$Decode$field, 'content', $elm$json$Json$Decode$string));
				},
				A2($elm$json$Json$Decode$field, 'name', $elm$json$Json$Decode$string));
		},
		A2($elm$json$Json$Decode$field, 'replayUrl', $elm$json$Json$Decode$string)))(0)}});}(this));